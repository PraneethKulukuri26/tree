...
def optimize_portfolio(request):
    ...
    try:
        ...
        rets_daily, mu, Sigma = fetch_and_prepare(tickers, period, interval)
        n = len(mu)
        ...
        best_overall, history = dinkelbach_qaoa_runner(mu, Sigma, risk_free, K)

        w_refined = subset_tangency(mu, Sigma, risk_free, best_overall['x'])
        s_refined = sharpe(w_refined, mu, Sigma, risk_free)
